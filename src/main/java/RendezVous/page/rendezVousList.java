...
public class rendezVousList extends JFrame {
    ...
    public rendezVousList(String nameD) {
        this.nameDoc = nameD;
        ...
        headerPanel.setBackground(new Color(70, 130, 180)); // MS blue
        headerPanel.setBorder(new EmptyBorder(10, 15, 10, 15));

        JLabel titleLabel = new JLabel("    Diagnostic List");
        titleLabel.setForeground(Color.WHITE);
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 18));
        headerPanel.add(titleLabel, BorderLayout.WEST);

        // Create refresh button panel
        JButton addButton = createModernButton("Close", new Color(120, 0, 25));
        ...
        String[] columnNames = {"ID", "Pation", "Docteur" , "Diagnostic", "Edit", "Delete"};
    ...
    private void loadData() {
        model.setRowCount(0);
        List<RendezVousSchema> data;// Clear existing data
        if (this.nameDoc !=""){
             data = F.paginate(this.nameDoc);
        }
        else {
             data = F.paginate("");
